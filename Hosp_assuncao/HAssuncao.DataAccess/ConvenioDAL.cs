    ...
    public class ConvenioDAL
    {
        ...
        public  List<Convenios> ListarConvenio()
        {
            ...
            while (reader.Read())
            {
                ...
                convenios.Spot = "";
                if (!Convert.IsDBNull(reader["spot"]))
                {
                    convenios.Spot = reader["spot"].ToString();
                }
                lista.Add(convenios);
            }
        ...
        public  int InserirConvenio(string descricao, string spot)
        {
            int Retorno;
            DataAccess.Conexao con = new DataAccess.Conexao(this.Con);
            con.AbrirConexao();
            string query;
            query="INSERT INTO Tb_Convenio (Descricao, Spot) values ('" + descricao + "', '" + spot + "')";
            Retorno = con.ExecutaComando(query);
            return Retorno;
        }
        public int AlterarConvenio(int codConvenio, string descricao, string spot)
        {
            int Retorno;
            DataAccess.Conexao con = new DataAccess.Conexao(this.Con);
            con.AbrirConexao();
            string query;
            query = "UPDATE Tb_CONVENIO SET Descricao='" + descricao + "', spot = '" + spot + "'";
