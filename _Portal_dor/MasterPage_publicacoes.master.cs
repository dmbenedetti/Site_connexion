...
public partial class MasterPage_grupo : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            var menu_ps = cnDor.TB_PORTAL_CONTEUDO.Where(x => x.id_status == 1 && x.id_sessao == 7 && x.id_idioma == 1 && x.ds_visivel == true && x.id_tipo != 12 && ((x.id_tipo > 0 && x.id_tipo <= 3) || x.id_tipo == 10) && x.id_hospital == 1).ToList();
            if (menu_ps.Count > 0)
            {
                string ps = "";
