    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            if (campanha != null)
            {
                ...
                Response.Write(@"<script>alert('Campanha não encontrada');location.href='portal_iep_iep.aspx'</script>");
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        int id_noticia = Convert.ToInt32(Request.QueryString["id_not"]);
            try
            {
                ...
                Response.Write(@"<script>alert('Notícia Excluida Com Sucesso');location.href='portal_iep_iep.aspx'</script>");
            }
            catch (Exception ex)
            {
