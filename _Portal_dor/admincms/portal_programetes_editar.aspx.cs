...
public partial class admincms_ogrupo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack) {
            ...
            if (video != null) {
                TextBox1.Text = video.nm_titulo;
                ltr_video.Text = video.ds_video;
                TextBox2.Text = video.ds_video;
            }
            else {
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        int id = Convert.ToInt32(Request.QueryString["id"]);
            ...
            if (video != null)
            {
                try
                {
                    video.nm_titulo = TextBox1.Text;
                    video.ds_video = TextBox2.Text;
                    cnDor.SaveChanges();
                    Response.Write(@"<script>alert('Dados alterados com sucesso.');location.href='portal_programetes.aspx'</script>");
                }
                catch (Exception ex)
                {
