...
public partial class admincms_usuarios_editar : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack) {
            ...
            chk_menus_portal.DataSource = cnDor.TB_ADMIN_MENU.Where(j => j.fg_portal == true).ToList();
            chk_menus_portal.DataTextField = "ds_menu";
            chk_menus_portal.DataValueField = "id_menu";
            chk_menus_portal.DataBind();

            chk_menus_hospitais.DataSource = cnDor.TB_ADMIN_MENU.Where(j => j.fg_portal == false).ToList();
            chk_menus_hospitais.DataTextField = "ds_menu";
            chk_menus_hospitais.DataValueField = "id_menu";
            chk_menus_hospitais.DataBind();
            ...
            if (usuario != null)
            {
                ...
                if (menus.Any())
                {
                    ...
                    {
                        var a = chk_menus_portal.Items.FindByValue(menu.id_menu.ToString());
                        if (a != null)
                            a.Selected = true;
                        var b = chk_menus_hospitais.Items.FindByValue(menu.id_menu.ToString());
                        if (b != null)
                            b.Selected = true;
                    }
                }
    ...
    protected void btn_gravar_Click(object sender, EventArgs e)
    {
        ...
        if (valida_login(txt_login.Text) == false)
        {
            ...
            try
            {
                ...
                for (int j = 0; chk_menus_portal.Items.Count > j; j++)
                {
                    if (chk_menus_portal.Items[j].Selected)
                    {
                        cnDor.AddToTB_ADMIN_USER_MENU(new db_rede_dorModel.TB_ADMIN_USER_MENU() { id_menu = Convert.ToInt32(chk_menus_portal.Items[j].Value), id_user = id_user });
                    }
                }
                for (int j = 0; chk_menus_hospitais.Items.Count > j; j++)
                {
                    if (chk_menus_hospitais.Items[j].Selected)
                    {
                        cnDor.AddToTB_ADMIN_USER_MENU(new db_rede_dorModel.TB_ADMIN_USER_MENU() { id_menu = Convert.ToInt32(chk_menus_hospitais.Items[j].Value), id_user = id_user });
                    }
                }
