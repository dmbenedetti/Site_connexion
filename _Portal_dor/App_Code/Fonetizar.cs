...
/// <summary>
/// Summary description for Fonetizar
/// </summary>
public static class iFonetizar
{
    ...
    private static string FonetizarParticula(string str)
    {
        ...
        if (aux[0].Equals('H'))
        {
            ...
            catch { aux2 = Convert.ToString(aux[0]); j = 1; }
        }
        else
        {
        ...
        try
        {
            sreturn = aux.Select(chr => letras.IndexOf(chr)).Aggregate(string.Empty, (current, n) => current + codFonetico[n]);
        }
        catch { sreturn = ""; }
        return sreturn;
    }
