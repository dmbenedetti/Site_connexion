...
public partial class admincms_ohospital_criar : System.Web.UI.Page
{
    int sessao_site = 8;//o hospital
    ...
    protected void grava_pagina_interna() {
        ...
        try
        {
            ...
            cnDor.SaveChanges();
            ClientScript.RegisterStartupScript(this.GetType(), "sucesso", "alert('Conteúdo gravado com Sucesso!');location.href='ohospital.aspx'", true);
        }
        catch (Exception ex)
        {
    ...
    protected void grava_link_interno()
    {
        ...
        try
        {
            ...
            cnDor.SaveChanges();
            ClientScript.RegisterStartupScript(this.GetType(), "sucesso", "alert('Link gravado com Sucesso!');location.href='ohospital.aspx'", true);
        }
        catch (Exception ex)
        {
            ClientScript.RegisterStartupScript(this.GetType(), "erro", "alert('erro na hora de gravar o conteudo \n " + ex.Message + "')", true);
    ...
    protected void grava_formulario()
    {
        ...
        try
        {
            cnDor.AddToTB_PORTAL_CONTEUDO(novoLinkInterno);
            cnDor.SaveChanges();
            ClientScript.RegisterStartupScript(this.GetType(), "sucesso", "alert('Formulário gravado com Sucesso!');location.href='ohospital.aspx'", true);
        }
        catch (Exception ex)
        {
            ClientScript.RegisterStartupScript(this.GetType(), "erro", "alert('erro na hora de gravar o conteudo \n " + ex.Message + "')", true);
    ...
    protected void grava_historico()
    {
        int idioma = Convert.ToInt32(Session.Contents["idioma"]);
        Boolean visivel;
        if (rdo_visivel.SelectedValue == "1") { visivel = true; } else { visivel = false; }
        ...
        novoLinkInterno.id_sessao = sessao_site;
        novoLinkInterno.id_idioma = idioma;
        novoLinkInterno.id_tipo = 11;//Histórico
        novoLinkInterno.dt_conteudo = DateTime.Now;
        novoLinkInterno.id_hospital = Convert.ToInt32(Session["idHosp"]);
        novoLinkInterno.nm_titulo_fonetico = iFonetizar.Fonetizar(novoLinkInterno.nm_titulo, false);
        novoLinkInterno.ds_resumo_fonetico = iFonetizar.Fonetizar(novoLinkInterno.ds_resumo, false);
        ...
        try
        {
            ...
            var conteudo = cnDor.TB_PORTAL_CONTEUDO.OrderByDescending(x => x.id_conteudo).Select(x=>x.id_conteudo).FirstOrDefault();
            ClientScript.RegisterStartupScript(this.GetType(), "sucesso", "location.href='historico.aspx?id="+conteudo+"'", true);
        }
        catch (Exception ex)
        {
            ClientScript.RegisterStartupScript(this.GetType(), "erro", "alert('erro na hora de gravar o conteudo \n " + ex.Message + "')", true);
