...
public partial class admincms_ogrupo_criar : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack) {
            ...
            DropDownList1.DataSource = cnDor.TB_FORMULARIOS.Where(x => x.id_hospital == 1 && x.id_status == 1).Select(x => new { x.ds_arquivo, x.nm_formulário }).ToList();
    ...
    protected void grava_pagina_interna(int id_conteudo) {
        try
        {
            ...
            if (pagina != null)
            {
                ...
                cnDor.SaveChanges();
                ClientScript.RegisterStartupScript(this.GetType(), "sucesso", "alert('Dados atualizados com sucesso!');location.href='portal_espaco_saude.aspx'", true);
            }//fim do if pagina!=null
        }
        catch (Exception ex)
        {
