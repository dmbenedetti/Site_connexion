...
public partial class admincms_ogrupo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack) {
            ...
            var optMenu = (from a in cnDor.TB_CONTEUDO_PAGINA_FIXA where a.id_sessao == 7 && a.id_hospital == idHosp && a.ds_link == "am_revistas.aspx" select a).FirstOrDefault();
            if (optMenu != null)
            {
    ...
    {
        Response.Redirect("am_revista_meddor_criar.aspx");
    }
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        {
            optMenu.ds_link = "am_revistas.aspx";
            optMenu.id_hospital = idHosp;
            optMenu.id_sessao = 7;
            cnDor.AddToTB_CONTEUDO_PAGINA_FIXA(optMenu);
        }
        cnDor.SaveChanges();
        var oMenuFixo = (cnDor.TB_PORTAL_MENUFIXO.Where(h => h.ds_linkmenu == ".aspx")).FirstOrDefault();
        if (oMenuFixo == null)
        {
            oMenuFixo = new db_rede_dorModel.TB_PORTAL_MENUFIXO();
            oMenuFixo.ds_linkmenu = "am_revistas.aspx";
        }
