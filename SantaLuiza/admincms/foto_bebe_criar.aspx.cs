    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (erro == false)
        {
            try
            {
                ...
                db_rede_dorModel.TB_FOTO_BEBE novaFoto = new db_rede_dorModel.TB_FOTO_BEBE();
                novaFoto.ds_foto_bebe = imagem2;
                novaFoto.ds_foto_retrato = imagem3;
                novaFoto.ds_foto_thumb = imagem;
                novaFoto.ds_nome_bebe = txt_NomeBebe.Text; ;
                novaFoto.ds_nome_mae = txtNomeMae.Text;
                novaFoto.ds_nome_pai = txtNomePai.Text;
                novaFoto.ds_obstreta = txtObstreta.Text;
                novaFoto.ds_pediatra = txtPediatra.Text;
                novaFoto.dt_cadastro = System.DateTime.Now;
                novaFoto.dt_nascimento = datanascimento;
                novaFoto.fg_status = 1;
                try
                {
                    novaFoto.nu_estatura = Convert.ToDouble(txtAltura.Text);
                }
                catch
                {
                ...
                try
                {
                    novaFoto.nu_peso = Convert.ToDouble(txtPeso.Text);
                }
                catch
                {
                    throw new Exception("Peso Inválida");
                }
                novaFoto.id_hospital = Convert.ToInt32(drpHospital.SelectedValue);
                cnDor.AddToTB_FOTO_BEBE(novaFoto);
