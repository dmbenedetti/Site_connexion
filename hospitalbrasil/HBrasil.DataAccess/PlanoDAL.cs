...
namespace HBrasil.DataAccess
{
    public class PlanoDAL
    {
        ...
        public List<Planos> TrocaLinhaColunaFrontEnd(int idConvenio)
        {
            List<Planos> lista = new List<Planos>();
            DataAccess.Conexao con = new DataAccess.Conexao(this.Con);
            con.AbrirConexao();
            string query;
            query = "sp_Planos_ById";

            SqlDataReader reader = con.RetornaDadosSP(query, new List<SqlParameter> { new SqlParameter { ParameterName = "@idConvenio", Direction = ParameterDirection.Input, DbType = DbType.Int32, Value = idConvenio } });
            Int32 idxPlano = reader.GetOrdinal("Plano"); 
            Int32 idxAmbulatorio = reader.GetOrdinal("AMBULATORIO");
            Int32 idxExames = reader.GetOrdinal("EXAMES");
            Int32 idxHospital = reader.GetOrdinal("HOSPITAL");
            ...
            {
                Planos plano = new Planos();
                plano.DescPlano = reader["Plano"].ToString();
                plano.Ambulatorio = Convert.ToInt32(reader["AMBULATORIO"].ToString());
                plano.Exames = Convert.ToInt32(reader["EXAMES"].ToString());
                plano.Hospital = Convert.ToInt32(reader["HOSPITAL"].ToString());
                plano.Maternidade = Convert.ToInt32(reader["MATERNIDADE"].ToString());
                plano.PS = Convert.ToInt32(reader["PS"].ToString());
                plano.CodConvenio = Convert.ToInt32(reader["CODCONVENIO"].ToString());
                plano.CodPlano = Convert.ToInt32(reader["CODPLANO"].ToString());
                plano.Descricao = reader["Convenio"].ToString();
                lista.Add(plano);
            }
            reader.Close();
            con.FechaConexao();
        ...
        public List<Planos> CarregaPlanos(string titulo)
        {
            ...
            if (titulo == string.Empty)
            {
                ...
                Int32 idxSpot = reader.GetOrdinal("Spot");
                while (reader.Read())
                {
                    Planos Plano = new Planos();
                    Plano.Descricao = reader.GetString(idxDescricao);
                    Plano.Spot = reader.GetString(idxSpot);
                    lista.Add(Plano);
