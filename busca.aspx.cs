...
public partial class busca : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            var resultado = (from j in
                                 (from v in cnDor.TB_PORTAL_CONTEUDO
                                  join w in cnDor.TB_PORTAL_CONTEUDO_HOSPITAL on v.id_conteudo equals w.id_conteudo into _a
                                  from a in _a.DefaultIfEmpty()
                                  select new
                                  {
                                      id_conteudo = v.id_conteudo,
                                      id_hospital = v.id_hospital,
                                      id_idioma = v.id_idioma,
                                      id_noticia = v.id_noticia,
                                      id_sessao = v.id_sessao,
                                      id_status = v.id_status,
                                      id_tipo = v.id_tipo,
                                      nm_titulo = v.nm_titulo,
                                      nm_titulo_fonetico = v.nm_titulo_fonetico,
                                      nr_edicaorevista = v.nr_edicaorevista,
                                      ds_conteudo = v.ds_conteudo,
                                      ds_conteudo_fonetico = v.ds_conteudo_fonetico,
                                      ds_imgDestaque = v.ds_imgDestaque,
                                      ds_resumo = v.ds_resumo,
                                      ds_resumo_fonetico = v.ds_resumo_fonetico,
                                      ds_url = v.ds_url,
                                      dt_conteudo = v.dt_conteudo,
                                      id_hospital_ls = a.id_hospital == null ? 0 : a.id_hospital
                                  }).ToList()
    ...
    protected string image(object imagem, int idSession)
    {
    ...
    protected string url(string id, int idTipo, int idSession)
    {
        string sURL = "";
        switch (idSession)
        {
            ...
            case 5:
                switch (idTipo) {
                    case 1:
                        sURL = "imprensa_page.aspx?id=" + id;
                        break;
                    case 5:
                        sURL = "noticias_detail.aspx?id=" + id;
                        break;
                    case 6:
                        sURL = "releases_detail.aspx?id=" + id;
                        break;
                }
                break;
            case 6:
